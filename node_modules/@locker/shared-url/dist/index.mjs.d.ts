interface ParsedURL {
    readonly normalizedURL: string;
    readonly pathname: string;
}
declare const TRUSTED_DOMAINS_REG_EXP: RegExp;
declare const URL_SCHEMES_LIST: string[];
declare const normalizerAnchor: HTMLAnchorElement;
declare function isValidURL(parsedURL: ParsedURL): boolean;
declare function isValidURLScheme(url: string): boolean;
declare function parseURL(url: string): ParsedURL;
declare function resolveURL(url: string): string;
declare function sanitizeURLForElement(url: string): string;
declare function sanitizeURLString(urlString: string): string;
declare function isMIMETypeAllowed(mimeType: string): boolean;
export { TRUSTED_DOMAINS_REG_EXP, URL_SCHEMES_LIST, normalizerAnchor, isValidURL, isValidURLScheme, parseURL, resolveURL, sanitizeURLForElement, sanitizeURLString, ParsedURL, isMIMETypeAllowed };

  ...
  connect() {
    ...
    this.router.subscribe((result) => {
      if (result.status === 404) {
        this.processError((0, import_routerErrors.generateMessageObject)(import_routerErrors.messages.DESTINATION_NOT_FOUND));
        return;
      }
      if (result.status && result.status >= 400) {
        const error = result.error || new Error();
        this.processError((0, import_routerErrors.generateMessageObject)(import_routerErrors.messages.DESTINATION_ERROR, [

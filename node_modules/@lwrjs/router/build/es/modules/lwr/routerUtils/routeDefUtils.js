...
export function matchRouteDefinitionByPageReference(pageReference, routeDefs) {
    ...
    if (refType) {
        ...
        const [matched] = routeDefs.filter((compiledDef) => {
            const { original: { page: { type = null, attributes: pageAttrs = {}, state: pageState = {} } = {} }, } = compiledDef;
            ...
            // exact value?
            const pageAttributeLiteralsCompletelyMatch = Object.keys(pageAttrs)
                .filter((key) => {
                const value = pageAttrs[key];
                return !value || !isParam(value);
            })
                .every((key) => pageAttrs[key] === refAttrs[key]);
            const pageStateLiteralsCompletelyMatch = Object.keys(pageState)
                .filter((key) => {
            ...
            return (matchesType &&
                refAttributesHasAllPageKeys &&
                refAttributesHasSameNumberOfKeys &&
                pageAttributeLiteralsCompletelyMatch &&
                refStateHasAllPageKeys &&
                pageStateLiteralsCompletelyMatch);
        });
        return matched || null;
    }
    // NO MATCH:
...
export function getPageReferenceFromUriAndRouteDef(uri, routeDef) {
    if (routeDef) {
        const { original: { page: { type: pageType = '', attributes: pageAttributes = {}, state: pageState = {} } = {}, } = {}, } = routeDef;
...
//# sourceMappingURL=routeDefUtils.js.map